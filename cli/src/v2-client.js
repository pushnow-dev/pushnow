...
export async function prepareMessageV2(config, directory, plaintext, { deviceIds, scheduledAt, expiresAt, sound, sourceKind, messageID = randomUUID() } = {}) {
  if(sourceKind!==undefined && !["web","cli","api","subscription"].includes(sourceKind)) throw new Error("Invalid source kind");
  validateSound(sound);
