...
export const productGuides: Record<string, ProductGuide> = {
  en: {
    ...
    steps: ['Sign in to PushNow on your iPhone and set up the device. Enable notifications if you want alerts outside the app; iOS settings determine how those alerts appear.', 'Start sender authorization with the PushNow CLI. Approve its code in the app and compare the sender fingerprint. Then verify the account fingerprint in the CLI before trusting the connection.', 'Send a small test message with a recognizable title and a useful link. Check that it appears in the correct inbox, then confirm notification behavior on the phone before relying on the workflow.', 'Keep sender credentials private and use separate senders for separate integrations. If a sender is no longer needed, revoke its access. Consult the API docs when adding scheduling, files, or retry handling.'],
  ...
  'zh-Hans': {
    ...
    steps: ['在 iPhone 上登录 PushNow 并完成设备设置。如果需要 App 外提醒，请允许通知；具体显示方式由 iOS 通知设置决定。', '通过 PushNow CLI 发起发送端授权，在 App 中核对授权码和发送端指纹；回到 CLI 后继续核对账号指纹，再信任这次连接。', '先发送标题容易辨认、附有实用链接的测试消息。确认内容进入正确的收件箱，再在手机上检查实际通知效果。', '妥善保管发送端凭据，为不同集成分别授权；停用的发送端应撤销权限。增加定时、文件或重试处理前，先查阅对应文档。'],
