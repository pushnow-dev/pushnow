...
发送 Key 与设备 ID 相互独立。Web 或 iOS 的“账户 → 发送密钥”支持为现有发送端创建第二个 Key、修改有效期或删除 Key。每个发送端最多保留 2 个有效 Key；重新生成会先让旧 Key 失效，再显示一个新的明文 Key。密钥只显示一次，需要立即复制或下载保存，并配合该发送端已有的加密凭据使用。
